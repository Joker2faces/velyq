/**
 * The previous per-observation odds writer, kept ONLY for measurement.
 *
 * Not in `src/`, so it cannot ship in the package or be imported by accident
 * from application code. Its sole consumer is
 * `odds-writer-batching.test.ts`, which needs a real before/after against the
 * same database rather than an argument from the shape of the current code --
 * and which fails if a future change reintroduces per-observation writes.
 *
 * Do not fix bugs here or extend it. It is a frozen baseline.
 */
...
import { eventIdentities, sports } from "../src/schema/catalog.js";
import {
  bookmakers,
  eventMarketOutcomes,
  eventMarkets,
 ...
 */
export const LEGACY_WIRED_ODDS_MARKET = "MATCH_WINNER_1X2" as const;
const WIRED_MARKET = LEGACY_WIRED_ODDS_MARKET;
...
export type LegacyReferenceData = Readonly<{
  sportId: string;
  providerId: string;
 ...
 */
export async function legacyEnsureFootballReferenceData(
  database: PrivilegedVelyqDatabase,
  providerCode: string,
): Promise<LegacyReferenceData> {
...
export type LegacyOddsIngestionRejection = Readonly<{
  ok: false;
  reason: "NO_EVENT_FOR_ODDS" | "MARKET_NOT_WIRED" | "UNMAPPED_SELECTION";
  row: NormalizedOdds;
}>;

export type LegacyOddsIngestionOutcome = Readonly<{
  ok: true;
  eventId: string;
  duplicate: boolean;
}>;
 ...
 */
export async function legacyIngestFootballOdds(
  database: PrivilegedVelyqDatabase,
  rows: readonly NormalizedOdds[],
  reference: LegacyReferenceData,
): Promise<
  readonly (LegacyOddsIngestionOutcome | LegacyOddsIngestionRejection)[]
> {
  ...
  const results: (LegacyOddsIngestionOutcome | LegacyOddsIngestionRejection)[] =
    [];
