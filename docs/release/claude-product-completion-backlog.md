...
| P0-D | Over/Under 2.5 (§18, §30) | **PARTIAL** | Writer, provider line parsing, storage, dedup, idempotency and settlement rules done and proven against real PostgreSQL; market identity determinism fixed in three places. **Remaining:** the decision engine and forecast cycle still produce only match-result forecasts, so no totals forecast reaches a customer. Totals prices are real, stored and settleable. |
