...
| P0-D | Over/Under 2.5 (§18, §30) | **PARTIAL** | Full pipeline now real: writer, provider line parsing, storage, dedup, idempotency, market identity determinism (3 sites fixed), real Dixon-Coles model probability, forecast, decision and settlement rules -- all proven end-to-end against real PostgreSQL (event market at line 2.5, real OVER/UNDER outcomes, persisted predictions summing to 1). **Remaining:** no customer surface shows a totals decision yet -- `selectOutcome` still restricts Today/EDGE/RADAR/Match Intelligence's headline card to the match-result outcome by design. A totals decision exists, prices, and will settle correctly, but nothing renders it. |
...
| # | Item | Status | Notes |
| --- | --- | --- | --- |
| P1-Q | Recompute on lineup arrival (§26) | OPEN | Evidence is now ingested with a before and an after. Nothing yet recomputes a forecast when a sheet lands, so What Changed and Evidence Timeline have no lineup event to show. |
