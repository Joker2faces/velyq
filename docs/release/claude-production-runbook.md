...
| Deployment | Note |
| --- | --- |
| `dpl_4UbktAQmzZuLjLfazGQ6tEnTcV7V` | **Current.** Freshness honesty (provider timestamp, live staleness gate, four-state DTO) and the whole-day quota simulations (code `5298c59`). Deployed 2026-09-10 15:46 EEST. Verified: health `LIVE`/`DATABASE`, ten routes 200 with `/api/v1/today` 401, seven security headers, `/el` 4376 Greek words, zero synthetic markers on `/today` |
