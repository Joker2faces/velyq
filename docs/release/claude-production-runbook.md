...
| Deployment | Note |
| --- | --- |
| `dpl_D9xaExEgRyDM5uEFXXtzHorVxmzW` | **Current.** Best-of-N bookmaker count: `currentOdds` was already the best price across every bookmaker at the latest instant (`bestAt()`), this makes it legible — `summariseOddsMovement` now returns `bookmakerCount`, threaded onto `CustomerMatchDto.bookmakerCount?` and shown as a "Best of N books" caption on Match Intelligence's price-validity card (code `6cde8f8`). Deployed 2026-09-10 18:21 EEST. Verified: health `LIVE`/`DATABASE`, ten routes 200 (`/matches/1` 307 as designed, unauth), six security headers, zero synthetic markers on `/today`. |
