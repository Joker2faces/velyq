...
| Deployment | Note |
| --- | --- |
| `dpl_8N3FWpMh2Zaqm953A17Q4bpx7BFg` | **Current.** "Why nothing today?": `CustomerTodayDto.summary` (totalFixtures, counts by recommendation, lineupGated) computed once in `mapToday` from the same fixture list Today already maps -- no second query, internal funnel-diagnostic route untouched. Today's empty state now cites it ("N fixtures today, M priced, K cleared EDGE") instead of a stock sentence (code `a7b5f6d`). Deployed 2026-09-10 19:02 EEST. Verified: health `LIVE`/`DATABASE`, ten routes 200, zero synthetic markers. |
