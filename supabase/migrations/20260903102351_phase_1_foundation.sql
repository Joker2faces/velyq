...
-- Cross-table canonical market identity cannot be expressed as a CHECK.
CREATE FUNCTION private.enforce_event_market_identity()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog
AS $$
DECLARE
  definition_requires_line boolean;
BEGIN
  SELECT line_required
  INTO definition_requires_line
  FROM market.market_definitions
  WHERE id = NEW.market_definition_id;

  IF FOUND AND definition_requires_line <> (NEW.line_value IS NOT NULL) THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'event market line presence must match its market definition';
  END IF;
...
REVOKE ALL ON FUNCTION private.enforce_event_market_identity() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.enforce_event_market_identity() TO postgres, service_role;

CREATE TRIGGER enforce_event_market_identity
BEFORE INSERT OR UPDATE OF market_definition_id, line_value
ON market.event_markets
FOR EACH ROW
EXECUTE FUNCTION private.enforce_event_market_identity();

CREATE FUNCTION private.enforce_event_market_outcome_identity()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog
AS $$
BEGIN
  IF NOT EXISTS (
    ...
    JOIN market.outcome_definitions AS outcome_definition
      ON outcome_definition.market_definition_id = event_market.market_definition_id
    WHERE event_market.id = NEW.event_market_id
      AND outcome_definition.id = NEW.outcome_definition_id
  ) THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'event market outcome must use an outcome from its market definition';
  END IF;
...
REVOKE ALL ON FUNCTION private.enforce_event_market_outcome_identity() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.enforce_event_market_outcome_identity() TO postgres, service_role;

CREATE TRIGGER enforce_event_market_outcome_identity
BEFORE INSERT OR UPDATE OF event_market_id, outcome_definition_id
...
GRANT EXECUTE ON FUNCTION private.reject_append_only_mutation() TO postgres, service_role;

CREATE TRIGGER reject_operations_source_observations_mutation
BEFORE UPDATE OR DELETE ON operations.source_observations
FOR EACH ROW EXECUTE FUNCTION private.reject_append_only_mutation();

CREATE TRIGGER reject_market_odds_observations_mutation
BEFORE UPDATE OR DELETE ON market.odds_observations
FOR EACH ROW EXECUTE FUNCTION private.reject_append_only_mutation();

CREATE TRIGGER reject_intelligence_data_quality_assessments_mutation
BEFORE UPDATE OR DELETE ON intelligence.data_quality_assessments
FOR EACH ROW EXECUTE FUNCTION private.reject_append_only_mutation();

CREATE TRIGGER reject_intelligence_predictions_mutation
BEFORE UPDATE OR DELETE ON intelligence.predictions
FOR EACH ROW EXECUTE FUNCTION private.reject_append_only_mutation();

CREATE TRIGGER reject_intelligence_prediction_inputs_mutation
BEFORE UPDATE OR DELETE ON intelligence.prediction_inputs
FOR EACH ROW EXECUTE FUNCTION private.reject_append_only_mutation();

CREATE TRIGGER reject_intelligence_score_results_mutation
BEFORE UPDATE OR DELETE ON intelligence.score_results
FOR EACH ROW EXECUTE FUNCTION private.reject_append_only_mutation();

CREATE TRIGGER reject_intelligence_radar_evidence_mutation
BEFORE UPDATE OR DELETE ON intelligence.radar_evidence
FOR EACH ROW EXECUTE FUNCTION private.reject_append_only_mutation();

CREATE TRIGGER reject_audit_admin_audit_events_mutation
BEFORE UPDATE OR DELETE ON audit.admin_audit_events
FOR EACH ROW EXECUTE FUNCTION private.reject_append_only_mutation();
...
GRANT EXECUTE ON FUNCTION private.set_profile_updated_at() TO postgres, service_role;
...
REVOKE ALL ON ALL TABLES IN SCHEMA audit, catalog, intelligence, market, operations, private FROM anon, authenticated;
REVOKE ALL ON ALL SEQUENCES IN SCHEMA audit, catalog, intelligence, market, operations, private FROM anon, authenticated;
REVOKE ALL ON ALL FUNCTIONS IN SCHEMA audit, catalog, intelligence, market, operations, private FROM anon, authenticated;

ALTER DEFAULT PRIVILEGES IN SCHEMA audit, catalog, intelligence, market, operations, private REVOKE ALL ON TABLES FROM anon, authenticated;
ALTER DEFAULT PRIVILEGES IN SCHEMA audit, catalog, intelligence, market, operations, private REVOKE ALL ON SEQUENCES FROM anon, authenticated;
ALTER DEFAULT PRIVILEGES IN SCHEMA audit, catalog, intelligence, market, operations, private REVOKE ALL ON FUNCTIONS FROM PUBLIC, anon, authenticated;

GRANT USAGE ON SCHEMA audit, catalog, intelligence, market, operations, private TO service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA audit, catalog, intelligence, market, operations, private TO service_role;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA audit, catalog, intelligence, market, operations, private TO service_role;

ALTER DEFAULT PRIVILEGES IN SCHEMA audit, catalog, intelligence, market, operations, private GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO service_role;
ALTER DEFAULT PRIVILEGES IN SCHEMA audit, catalog, intelligence, market, operations, private GRANT USAGE, SELECT ON SEQUENCES TO service_role;
