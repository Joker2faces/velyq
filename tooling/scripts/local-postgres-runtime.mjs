...
/**
 * Shared so the browser suite can borrow the same database.
 *
 * `tooling/e2e/admin-web-server.mjs` needs a migrated, seeded PostgreSQL --
 * the admin journey traces specific seeded UUIDs from provider run to
 * prediction, score and quality. Nothing provisioned one, so
 * `pnpm test:e2e` failed the admin project with a confusing "heading not
 * found" rather than "there is no database", and the journey looked like a
 * product defect when it was a missing prerequisite.
 */
export { connectionString, failureLog, bootstrap, stop, wsl, run };
