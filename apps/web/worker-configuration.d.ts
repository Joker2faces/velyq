/* eslint-disable */
// Generated by Wrangler by running `wrangler types` (hash: 25a978cf3bd5612b683a7ae2d19e6587)
// Runtime types generated with workerd@1.20260903.1 2026-09-06 nodejs_compat
interface __BaseEnv_Env {
  VELYQ_RATE_LIMIT: KVNamespace;
...
interface Env extends __BaseEnv_Env {}
/*
 * `wrangler types` generates a `NodeJS.ProcessEnv` augmentation here that
 * narrows each plain-text var (e.g. `NEXT_PUBLIC_SUPABASE_URL`) to the exact
 * literal string configured in wrangler.jsonc's `vars`. That is accurate for
 * `env.X` inside a deployed Worker, but `process.env` is a live, mutable
 * object every test in this workspace legitimately reassigns to a different
 * string for isolation — e.g. `process.env["NEXT_PUBLIC_SUPABASE_URL"] =
 * "https://supabase.test"`. A literal-narrowed `ProcessEnv` makes every one
 * of those assignments a type error. This block is deliberately omitted;
 * `@types/node`'s own `ProcessEnv` (an untyped string index) already covers
 * `process.env` correctly. Re-apply this removal if `worker-configuration.d.ts`
 * is regenerated — see `apps/web/test/worker-configuration.test.ts` and this
 * comment for why.
 */
